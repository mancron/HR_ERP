package com.hrms.att.controller;

import com.hrms.att.service.OvertimeService;
import com.hrms.emp.dto.EmpDTO;

import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.*;

import java.io.IOException;

@WebServlet("/att/overtime/cancel")
public class OvertimeCancelServlet extends HttpServlet {

    private OvertimeService overtimeService = new OvertimeService();

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        request.setCharacterEncoding("UTF-8");

        HttpSession session = request.getSession();
        EmpDTO loginUser = (EmpDTO) session.getAttribute("loginUser");

        // 🔒 로그인 체크
        if (loginUser == null) {
            response.sendRedirect(request.getContextPath() + "/auth/login.do");
            return;
        }

        int empId = loginUser.getEmp_id();

        String month = request.getParameter("month");

        try {
            // 🔥 파라미터
            int id = Integer.parseInt(request.getParameter("id"));

            // 🔥 Service 호출
            boolean result = overtimeService.cancelOvertime(id, empId);

            String redirectUrl = request.getContextPath() + "/att/overtime/req";

            if (result) {
                redirectUrl += "?msg=cancel_success";
            } else {
                redirectUrl += "?error=cancel_fail";
            }

            // 🔥 month 유지
            if (month != null) {
                redirectUrl += "&month=" + month;
            }

            response.sendRedirect(redirectUrl);

        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect(request.getContextPath() + "/att/overtime/req?error=exception");
        }
    }
}