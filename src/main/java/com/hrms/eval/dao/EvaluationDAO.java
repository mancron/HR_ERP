...
public class EvaluationDAO {
     ...
     * rejectReason: HR담당자가 입력한 반려 사유 (빈 문자열 허용)
     *
     * 저장 형식: "[반려] 기존코멘트\n[반려 사유] 사유내용"
     * 중복 반려 차단: 이미 [반려] 태그가 있으면 return -1
     *
    ...
    /**
     * 요약 통계 — 검색 파라미터 포함 버전
     * status.jsp 이름 검색과 무관하게 연도/기간/유형 기준으로 집계하는 기본 버전과
     * 검색 조건도 반영하는 버전을 모두 제공
     */
    public Map<String, Integer> getEvaluationSummary(int year, String period, String type,
                                                       String searchTarget, String searchEvaluator) {
        return getEvaluationSummary(year, period, type); // 현재는 검색조건 무관 전체 집계 유지
        // NOTE: 이름 검색까지 summary에 반영하려면 아래 로직 분리 필요
        // 현재 정책: summary 카드는 필터(연도/기간/유형) 기준 전체, 이름 검색은 테이블만 적용
    }
     ...
     * 평가 유형별 대상자 목록
     * - 자기평가: 본인만
     * - 상위평가: position_level < 내 레벨 (하위직급)
     * - 동료평가: position_level = 내 레벨 (동일직급, 본인 제외)
     * - 하위평가: position_level > 내 레벨 (상위직급 — 사원이 부장 평가 등 역방향)
     * - posLevel=0(직급 없음) 또는 최고직급자(하위가 없음): 상위평가 시 전체 재직자 반환
     */
    public Vector<Map<String, Object>> getEmployeeListForEvaluator(
        ...
        if ("자기평가".equals(evalType)) {
            ...
            // 대상자가 나보다 상위직급(레벨 숫자가 큰 쪽이 하위, 시스템마다 다르므로 > 사용)
            sql = "SELECT e.emp_id, e.emp_name, COALESCE(p.position_name,'사원') AS pos "
                + "FROM employee e "
                + "LEFT JOIN job_position p ON e.position_id = p.position_id "
                + "WHERE e.status='재직' AND e.emp_id != ? AND p.position_level > ? "
            ...
            // 상위평가 (기본)
            // 최고직급자(posLevel이 DB 내 최솟값)이면 하위직급자가 없어 빈 목록이 됨
            // → position_level < posLevel 조건에 걸리는 사람이 없는 경우를 대비해
            //   posLevel=0이거나 전체조회 플래그(999)면 본인 제외 전체 반환
