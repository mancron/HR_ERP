...
public class DeptDAO {
	...
	public List<DeptDTO> getAllDepts() {
		...
		try {
			con = DatabaseConnection.getConnection();
			String sql = "SELECT d.dept_id, d.dept_name, d.parent_dept_id, d.manager_id, "
					+ "       d.dept_level, d.sort_order, d.is_active, d.closed_at, d.created_at, "
					+ "       e.emp_name AS manager_name " + "FROM department d "
					+ "LEFT JOIN employee e ON e.emp_id = d.manager_id " + "ORDER BY d.sort_order ASC, d.dept_id ASC";
	...
	public DeptDTO getDeptById(int deptId) {
		...
		try {
			...
			String sql = "SELECT d.dept_id, d.dept_name, d.parent_dept_id, d.manager_id, "
					+ "       d.dept_level, d.sort_order, d.is_active, d.closed_at, d.created_at, "
					+ "       e.emp_name AS manager_name " + "FROM department d "
					+ "LEFT JOIN employee e ON e.emp_id = d.manager_id " + "WHERE d.dept_id = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, deptId);
			rs = pstmt.executeQuery();
			if (rs.next())
	...
	public List<DeptDTO> getChildDepts(int parentId) {
		...
		try {
			...
			String sql = "SELECT d.dept_id, d.dept_name, d.parent_dept_id, d.manager_id, "
					+ "       d.dept_level, d.sort_order, d.is_active, d.closed_at, d.created_at, "
					+ "       e.emp_name AS manager_name " + "FROM department d "
					+ "LEFT JOIN employee e ON e.emp_id = d.manager_id " + "WHERE d.parent_dept_id = ?";
			pstmt = con.prepareStatement(sql);
	...
	/**
	 * 부서 신규 등록 (manager_id 제외 — 인사발령에서 관리) [FK FIX] parent_dept_id = 0 이면 NULL 저장
	 * (FK 제약 준수)
	 */
	...
	/**
	 * 부서 수정 (manager_id 제외 — 인사발령에서 관리) [FK FIX] parent_dept_id = 0 이면 NULL 저장 (FK
	 * 제약 준수) is_active = 1 이면 closed_at 을 NULL 로 초기화
	 */
	public boolean updateDept(DeptDTO dept) {
		...
		String sql = "UPDATE department SET dept_name = ?, parent_dept_id = ?, "
				...
				+ "closed_at = (CASE WHEN ? = 1 THEN NULL ELSE closed_at END) " + "WHERE dept_id = ?";
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, dept.getDept_name());
	...
	public List<Map<String, Object>> getMembersByDeptId(int deptId) {
		...
		try {
			con = DatabaseConnection.getConnection();
			String sql = "WITH RECURSIVE DeptHierarchy AS ( " + "    SELECT dept_id FROM department WHERE dept_id = ? "
					+ "    UNION ALL " + "    SELECT d.dept_id FROM department d "
					+ "    INNER JOIN DeptHierarchy dh ON d.parent_dept_id = dh.dept_id " + ") "
					+ "SELECT e.emp_no, e.emp_name, e.status, " + "COALESCE(p.position_name, '미지정') AS position_name "
	...
	public boolean deleteDept(int deptId) {
		if (hasActiveMembersRecursive(deptId))
			return false;
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement("UPDATE department SET is_active = 0, closed_at = NOW() WHERE dept_id = ?");
			pstmt.setInt(1, deptId);
			return pstmt.executeUpdate() > 0;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean hasActiveMembersRecursive(int deptId) {
		...
		try {
			con = DatabaseConnection.getConnection();
			String sql = "WITH RECURSIVE SubDepts AS ( " + "    SELECT dept_id FROM department WHERE dept_id = ? "
					+ "    UNION ALL " + "    SELECT d.dept_id FROM department d "
					+ "    INNER JOIN SubDepts sd ON d.parent_dept_id = sd.dept_id " + "    WHERE d.is_active = 1 "
					+ ") " + "SELECT COUNT(*) FROM employee " + "WHERE dept_id IN (SELECT dept_id FROM SubDepts) "
					+ "AND status = '재직'";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, deptId);
			rs = pstmt.executeQuery();
	...
	public List<DeptDTO> getInactiveDepts() {
		...
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement("SELECT dept_id, dept_name, closed_at FROM department "
	...
	public int getMemberCount(int deptId) {
		...
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement("SELECT COUNT(*) FROM employee WHERE dept_id = ? AND status != '퇴직'");
			pstmt.setInt(1, deptId);
			rs = pstmt.executeQuery();
			if (rs.next())
				return rs.getInt(1);
	...
	public int getChildDeptCount(int deptId) {
		...
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement("SELECT COUNT(*) FROM department WHERE parent_dept_id = ? AND is_active = 1");
			pstmt.setInt(1, deptId);
			rs = pstmt.executeQuery();
			if (rs.next())
				return rs.getInt(1);
	...
	public List<Map<String, Object>> findDeptIdByEmpName(String searchVal) {
		...
		try {
			con = DatabaseConnection.getConnection();
			String sql = "SELECT e.dept_id, d.dept_name, p.position_name, e.emp_name, e.emp_no " + "FROM employee e "
	...
	public boolean updateDeptLevel(int deptId, int newLevel) {
		...
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement("UPDATE department SET dept_level = ? WHERE dept_id = ?");
	...
	public boolean isManager(int empId) {
		...
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement("SELECT COUNT(*) FROM department WHERE manager_id = ? AND is_active = 1");
	...
	public void shiftSortOrder(int parentId, int oldOrder, int newOrder) {
		...
		if (oldOrder == 0 || oldOrder == newOrder) {
			sql = "UPDATE department SET sort_order = sort_order + 1 WHERE " + parentCondition
					+ " AND sort_order >= ? AND is_active = 1";
		} else if (oldOrder > newOrder) {
			sql = "UPDATE department SET sort_order = sort_order + 1 WHERE " + parentCondition
					+ " AND sort_order >= ? AND sort_order < ? AND is_active = 1";
		} else if (oldOrder < newOrder) {
			sql = "UPDATE department SET sort_order = sort_order - 1 WHERE " + parentCondition
					+ " AND sort_order > ? AND sort_order <= ? AND is_active = 1";
		} else {
	 ...
	 * [추가] 감사 로그 기록 메서드
	 */
	public void insertAuditLog(Integer actorId, String action, String tableName, int targetId, String columnName,
			String oldValue, String newValue) {
		Connection con = null;
		PreparedStatement pstmt = null;

// 1. 모든 컬럼을 포함하도록 SQL 수정
		String sql = "INSERT INTO audit_log (actor_id, action, target_table, target_id, column_name, old_value, new_value, created_at) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
		try {
			con = DatabaseConnection.getConnection();
			pstmt = con.prepareStatement(sql);
// actor_id 처리
			...
			pstmt.setString(2, action); // INSERT, UPDATE, DELETE 등
			pstmt.setString(3, tableName); // department
			pstmt.setInt(4, targetId); // 부서 ID
			pstmt.setString(5, columnName); // 변경된 컬럼명 (없으면 null)
			pstmt.setString(6, oldValue); // 변경 전 값 (없으면 null)
			pstmt.setString(7, newValue); // 변경 후 값 또는 요약 정보
			pstmt.executeUpdate();
		} catch (Exception e) {
			System.err.println("!!! 감사 로그 삽입 실패 !!!");
			e.printStackTrace();
		} finally {
			closeResources(null, pstmt, con);
		}
